...
x = [Symbol(f"x{i}", BOOL) for i in I]
y = [Symbol(f"y{i}", BOOL) for i in I]
...
for t in I:
    for s in I:
        ...
        if len(P[t]) != 0 and len(P[s]) != 0 and len(P[t] & P[s]) == 0:
            # this essentially means that they cannot both be one
            # intersecting_ineq.append(y[t] + y[s] <= 1)
            intersecting_ineq.append(Or(Not(y[t]), Not(y[s])))
...
star_ineq = []
# TODO: count booleans instead of integers
# TODO: convert cardinality constraints to boolean constraints (ie sums)
